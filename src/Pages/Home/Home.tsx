...
const Home = () => {
  ...
  const deleteFavorite = async (recipeID: string) => {
    ...
    if (favoritesDeleteResponse.error) {
      ...
      setRecipes(
        recipes?.map((recipe) => {
          const newFavoritesWithoutRecipe = recipe.Favorites.filter(
            (fav) => fav.recipe_id !== recipeID
          );
          return recipe.id === recipeID
            ? { ...recipe, Favorites: newFavoritesWithoutRecipe }
            : recipe;
        })
      );
    }
